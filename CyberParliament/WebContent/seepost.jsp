...
<script type="text/javascript">
	...
	function disablelike() {
		...
		if (hidden) {
			...
			document.forms["form"]["like"].style.visibility = 'hidden';
		} else {

			document.forms["form"]["unlike"].style.visibility = 'visible';
		}
		return false;
	}
	function disableunlike() {
		hidden = !hidden;
		if (hidden) {
			document.forms["form"]["unlike"].style.visibility = 'hidden';
		} else {

			document.forms["form"]["like"].style.visibility = 'visible';
		}
		return false;
	}
</script>
...
<body>
	<s:if test="#session.username!=null">
		...
		<div class="main">
			...
			<!-- -->
			<s:if test="#session.username!=null">
				<s:iterator var="e" value="posttohome">
					<div
						...
						DEPARTMENT:<a href=""><s:property value="#e.department" /></a> <br>
						DATE:
						<s:property value="#e.postdate" />
						<br>
						<textarea><s:property value="#e.post" /></textarea>
						<br>
						<form action="like" method="post" name="form"
							onsubmit="return disableunlike()">
							<input type="radio" value="<s:property value="#e.post"/>"
								name="post" id="like" checked="checked" style="display: none;" />
							<input type="submit" value="like" id="likesubmit" />
						</form>
						(
						<s:property value="#e.likes" />
						-People liked )<br>
						<form action="unlike" method="post" name="form1"
							onsubmit="return disablelike()">
							<input type="radio" value="<s:property value="#e.post"/>"
								name="post" id="unlike" checked="checked" style="display: none;" />
							<input type="submit" value="unlike" id="unlikesubmit"
								style="display: none;">
						</form>
